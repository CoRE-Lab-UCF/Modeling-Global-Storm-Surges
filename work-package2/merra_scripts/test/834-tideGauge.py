...
def extract_data(delta= 3):
    ...
    # #assign empty dataframes for predictors    
    # finalSLP = pd.DataFrame(columns = list(range(len(ind_grids))));
    # finalUwnd = pd.DataFrame(columns = list(range(len(ind_grids))));
    # finalVwnd = pd.DataFrame(columns = list(range(len(ind_grids)))); 
    
    #to mark the firs csv
    firstCsv = True;
