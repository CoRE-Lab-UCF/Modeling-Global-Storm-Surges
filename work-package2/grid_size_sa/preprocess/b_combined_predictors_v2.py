# -*- coding: utf-8 -*-
"""
Created on Tue Apr 16 16:11:00 2020
--------------------------------------------
Load predictors for each TG and combine them
--------------------------------------------

@author: Michael Tadesse
"""

import os 
import pandas as pd

dir_in = 'F:\eraint_D5'
dir_out = 'F:\eraint_combined_predictors'

#looping through tide gauges
os.chdir(dir_in)
for tg in os.listdir():
    print(tg)
    
    #looping through each TG folder
    os.chdir(tg)
    
    #defining the path for each predictor
    where = os.getcwd()
    csv_path = {'prcp' : os.path.join(where, 'prcp.csv'), \
               'slp' : os.path.join(where, 'slp.csv'),\
               'sst' : os.path.join(where, 'sst.csv'), \
               "wnd_u": os.path.join(where, 'wnd_u.csv'),\
               'wnd_v' : os.path.join(where, 'wnd_v.csv')}
        
    first = True   
    for pr in csv_path.keys():
        print(pr)
          
        #read predictor
        pred = pd.read_csv(csv_path[pr])
        
        #remove unwanted columns
        pred.drop(['Unnamed: 0', 'Unnamed: 0.1'], axis = 1, inplace=True)
        
        #get 6hrly values from prcp
        if pr == 'prcp':
            date_filter = lambda x: x.endswith('00:00:00') or \
                x.endswith('06:00:00') or x.endswith('12:00:00') \
                    or x.endswith('18:00:00')
            date_ind = list(map(date_filter, pred['date']))
            pred = pred.loc[date_ind]
            
        #give predictor columns a name
        pred_col = list(pred.columns)
        for pp in range(len(pred_col)):
            if pred_col[pp] == 'date':
                continue
            pred_col[pp] = pr + str(pred_col[pp])
        pred.columns = pred_col
        
        #merge all predictors
        if first:
            pred_combined = pred
            first = False
        else:
            pred_combined = pd.merge(pred_combined, pred, on = 'date')
        
    #saving pred_combined
    os.chdir(dir_out)
    pred_combined.to_csv('.'.join([tg, 'csv']))
    os.chdir(dir_in)
    print('\n')