...
Modified on Thu Dec 09 16:57:00 2021
...
def getTrends(recon, year, threshold, ci, mnkd, *args):
    ...
    for tg in tgList:
        ...
        #############################################################
        # special case with 1900-1950
        if original_year == "1900_1950":
            # filter only 1900-1950 data
            dat = dat[(dat['year'] >= 1900) & (dat['year'] <= 1950)]
            # adjust indices
            dat.reset_index(inplace = True)
            # set year = 1900 manually
            year = 1900
        #############################################################
    ...
    # save file
    saveDir = "\\003-" + str(original_year) + "Trends" + "\\{}".format(threshold)
    os.chdir(dirOut[recon] + saveDir)
    df.to_csv("{}_{}_{}thPercTrends_reg_{}CI_{}_{}.csv".format(recon, original_year, threshold, ci, mnkd, ar)) # adding heteroscedasticity-consistent standard errors
...
# fun function
getTrends("twcr", 1930, 95, 95, "modifiedMK", 'HAC')