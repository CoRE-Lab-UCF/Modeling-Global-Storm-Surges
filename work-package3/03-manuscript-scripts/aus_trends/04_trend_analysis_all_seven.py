"""
Created on Sun Jan 16 12:22:00 2022
Modified on Fri Jan 21 12:28:00 2022
...
dir_home = "G:\\report\\year-3\\07-Fall-2020\\#3Paper\\data"\
            "\\trend-analysis\\data\\allSevenTrends\\rawData\\"\
                    "australia_tgs\\percentile_merged"
...
def main():
    ...
    df = pd.DataFrame(columns = ['tg', 'lon', 'lat', 'tobs', 't20cr', 'te20c',
                                         'teint', 'tmer', 'te5', 'tens',
                                        'pobs', 'p20cr', 'pe20c', 'peint', 'pmer', 'pe5', 'pens' ])
    ...
    for tg in os.listdir():
        ...
        if (start_year != 1980) | (end_year != 2010):
            print("doesnt start in 1980 or end in 2010")
            continue

        # compute trends
        # check 75% completeness
        if len(dat) < 0.75*(2011 - 1980):
            print('75 percent completion not fulfilled')
            continue
        ...
        newDf.columns = ['tg', 'lon', 'lat', 'tobs', 't20cr', 'te20c', 'teint', 'tmer', 'te5', 'tens',
                                            'pobs', 'p20cr', 'pe20c', 'peint', 'pmer', 'pe5', 'pens']
    ...
    # save data 
    os.chdir(dir_out)
    df.to_csv('aus_SevenTrends_99th.csv')
