...
dirHome = "G:\\report\\year-3\\07-Fall-2020\\#3Paper\\data\\"\
    "trend-analysis\\data\\04-regionalStormFreq"
...
def getTrends(year, *args):
    ...
    df = pd.DataFrame(columns = ['tg', 'normality', 'trend(storms/year)', 'intercept','pval', 'dw', 'stderr'])
    ...
    for tg in tgList:
        ...
        shapiro, dagosto = checkNormality(dat['avg'])
        ...
        if (shapiro > 100) or (dagosto > 100):
            ...
            newDf = pd.DataFrame([tg, False, np.nan, np.nan]).T
            newDf.columns = ['tg', 'normality', 'trend(storms/year)', 'intercept', 'pval', 'dw', 'stderr']
