...
b_p = 'D:\OneDrive - Knights - University of Central Florida\Daten\MLR\MLR_K_Fold4x4'
lst = dir('*.mat');
rng default %to get same randomized results always
for i_i = 118:length(lst)
    load(lst(i_i).name)
    clearvars -except i_i baseFileName b_p lst vars y_surge lat_t lon_t
    INDICES = crossvalind('Kfold',y_surge,10); %partitioning the data to k fold
    ...
    f2 = sprintf('%s_mlr.mat', char(f1(1)));
    save(f2);
