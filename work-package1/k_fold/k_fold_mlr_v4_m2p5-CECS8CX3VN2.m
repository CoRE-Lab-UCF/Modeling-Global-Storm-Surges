...
b_p = 'F:\OneDrive - Knights - University of Central Florida\Daten\MLR\Model_1\03_27_2019\M1_MLR_Kfold_10X10_17yrs' %MLR folder
cd 'F:\OneDrive - Knights - University of Central Florida\Daten\MLR\Model_1\03_27_2019\PCA_Stepwise_confg_13_10X10_17yrs' %PCA folder
lst = dir('*.mat');
rng default %to get same randomized results always
for i_i = 1:length(lst)
    cd 'F:\OneDrive - Knights - University of Central Florida\Daten\MLR\Model_1\03_27_2019\PCA_Stepwise_confg_13_10X10_17yrs' %PCA folder
    load(lst(i_i).name)
    clearvars -except i_i baseFileName b_p lst vars y_surge lat_t lon_t indx list_tg
    INDICES = crossvalind('Kfold',y_surge(:,2),10); %partitioning the data to k folds
    ...
    for a = 1:10
        ...
        y_trn = y_surge(ind_trn,2); %training predictand/storm surge 
        ...
        y_tst = y_surge(ind_tst,2); %testing predictand
