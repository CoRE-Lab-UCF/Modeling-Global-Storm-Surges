...
cd 'F:\OneDrive - Knights - University of Central Florida\Daten\MLR\PCA_Stepwise_confg_8'
...
pool = [680 78 801 218 854 378]';  %to test a few gauges
...
for pp = 1:length(pool)%t = 1:3%length(list_tg)
    ...
    [cof_uv,score_uv,latent_uv,tsquared_uv,explained_uv] = pca(z_uv);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    %uv
    ee = 1;
    while sum(explained_uv(1:ee)) < 90
        ee = ee + 1;
    end
    uv_pc = ee; % number of PCs chosen for uwnd
    pc_uv = score_uv(:,1:uv_pc);
    ...
    vars = [pc_uwnd pc_vwnd pc_uwnd2 pc_vwnd2 pc_uv];
    y_surge = surged(:,2);
    %y_skew = skewd(:,2);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    % Saving .mat files
    folder = 'F:\OneDrive - Knights - University of Central Florida\Daten\MLR\PCA_Stepwise_confg_8'
