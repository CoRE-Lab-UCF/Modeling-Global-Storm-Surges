...
cd 'F:\OneDrive - Knights - University of Central Florida\Daten\MLR\PCA_Stepwise_confg_3'
...
for t = 1:length(list_tg)
    ...
    load('CCMP.mat'); clearvars -except basepath list_tg t Twind umax_daily vmax_daily fid lat_t lon_t
    pcd = dir('*_sk.mat'); % loading predictand file, file ends with _sk
    load(pcd.name);
    clearvars -except basepath list_tg t Twind umax_daily vmax_daily sk_daily surge_daily lat_t lon_t
    
    disp('1.12 - Loading precipitation data')
    load('GPCP.mat'); 
    clearvars -except basepath list_tg t Twind umax_daily ... 
        vmax_daily sk_daily surge_daily Tgpcp gpcp_daily lat_t lon_t
    
    disp('1.13 - Loading sea level pressure data')
    load('PRMSL.mat'); 
    clearvars -except basepath list_tg t Twind umax_daily lat_t lon_t ... 
    ...
    if min(b) < max(a)
        cd 'F:\OneDrive - Knights - University of Central Florida\Daten\MLR\PCA_Stepwise'
    ...
    clearvars -except basepath list_tg t Twind u1 u2 u3 v1 v2 v3 s1 s2 s3 g1 g2 g3 ... 
    ...
    % Compute zscores 
    z_u = zscore(u_rsh2); z_v = zscore(v_rsh2); z_s = zscore(s_rsh2);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    %sst
    ee = 1;
    if isempty(find(isfinite(explained_s)))
        pc_sst = zeros(length(score_s),1); % make it 0 if its nan
    else
    ...
    %clearvars -except basepath list_tg t Twind Tgpcp Tprmsl Tsst ...
        ...
        %pc_sst pc_prmsl pc_gpcp fid lat_t lon_t
    
    vars = [pc_uwnd pc_vwnd pc_sst pc_prmsl pc_gpcp];
    y_surge = surged(:,2);
    y_skew = skewd(:,2);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    % Saving .mat files
    folder = 'F:\OneDrive - Knights - University of Central Florida\Daten\MLR\PCA_Stepwise'
    baseFileName = sprintf('%s_pca_stp.mat', list_tg(t).name);
    fullMatFileName = fullfile(folder, baseFileName);
    save(fullMatFileName);
    clearvars -except basepath list_tg t fid
end
fclose(fid);
