...
b_pat =  'F:\OneDrive - Knights - University of Central Florida\Daten\MLR\TG_pct_pcd_17yrs'
list_tg = dir(b_pat); list_tg(1:2) = []; % making a list for the tg folders

%pool = [159 680 78 801 218 854 378]';  %to test a few gauges

for t = 1:length(list_tg)%pp = 1:length(pool)%t = 1:3%length(list_tg)
    cd(fullfile(b_pat, list_tg(t).name))
    t
    disp(list_tg(t).name)
    list_pp = dir('*.mat'); % making a list of the predictand and predictor files
    
    disp('1.11 - Loading wind data')
    load('CCMP.mat'); clearvars -except pp pool b_pat list_tg t Twind umax_daily vmax_daily fid lat_t lon_t
    pcd = sprintf('%s_daily.mat', list_tg(t).name); % loading predictand file, file ends with _sk
    
    load(pcd);  
    clearvars -except baseFileName pp pool b_pat list_tg t Twind umax_daily vmax_daily sk_daily surge_daily lat_t lon_t
    
    disp('1.12 - Loading precipitation data')
    load('GPCP.mat'); 
    clearvars -except pp pool b_pat list_tg t Twind umax_daily ... 
        vmax_daily sk_daily surge_daily Tgpcp gpcp_daily lat_t lon_t
    
    disp('1.13 - Loading sea level pressure data')
    load('PRMSL.mat'); 
    clearvars -except pp pool b_pat list_tg t Twind umax_daily lat_t lon_t ... 
    ...
    a = [tw(1,1) ts(1,1) tg(1,1) tp(1,1) surge_daily(1,1)]; %finding starting date
    b = [tw(end,1) ts(end,1) tg(end,1) tp(end,1) surge_daily(end,1)];
    dat = (max(a):min(b))'; % creating continuous vector of time
    
    if min(b) < max(a)
        cd 'F:\OneDrive - Knights - University of Central Florida\Daten\MLR\PCA_Stepwise_confg_13_10X10_17yrs'
    ...
    clearvars -except pp pool b_pat list_tg t Twind u1 u2 u3 v1 v2 v3 s1 s2 s3 g1 g2 g3 ... 
    ...
    %clearvars -except b_pat list_tg t Twind Tgpcp Tprmsl Tsst ...
    ...
    [cof_s,score_s,latent_s,tsquared_s,explained_s] = pca(z_s);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    pc_prmsl = score_p(:,1:p_pc);
      ...
      %sst
    ee = 1;
    while sum(explained_s(1:ee)) < 90
        ee = ee + 1;
    end
    s_pc = ee;
    pc_sst = score_s(:,1:s_pc);
    ...
    g_pc = ee;
    pc_gpcp = score_g(:,1:g_pc);
    

    vars = [pc_uwnd pc_vwnd pc_uwnd2 pc_vwnd2 pc_uwnd3 pc_vwnd3 pc_prmsl pc_sst pc_gpcp];
    y_surge = surged;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    %clearvars -except b_pat list_tg t Twind Tgpcp Tprmsl Tsst lat_t lon_t ...
        %umaxd vmaxd sstd gpcpd prmsld surged skewd pc_uwnd pc_vwnd ...
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    %cd(fullfile(b_pat, list_tg(t).name))
    %load('CCMP.mat'); clearvars -except b_pat list_tg t Twind Tgpcp Tprmsl Tsst ...
    ...
    % Saving .mat files
    clearvars -except baseFileName vars y_surge  lat_t lon_t  pp pool b_pat list_tg t fid
    folder = 'F:\OneDrive - Knights - University of Central Florida\Daten\MLR\PCA_Stepwise_confg_13_10X10_17yrs_v2'
    baseFileName = sprintf('%s_pca_stp_10x10_17yrs.mat', list_tg(t).name);
    fullMatFileName = fullfile(folder, baseFileName);
    save(fullMatFileName);
    clearvars -except pp pool b_pat list_tg t fid
end
fclose(fid);
