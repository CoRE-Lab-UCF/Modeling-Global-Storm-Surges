...
basepath =  'F:\OneDrive - Knights - University of Central Florida\Daten\Tide_data\TG_testing'
% list_tg = dir(basepath); 
% list_tg(1:2) = []; % making a list for the tg folders

%pool = [159 680 78 801 218 854 378]';  %to test a few gauges

for t = 1:1%length(list_tg)%pp = 1:length(pool)%t = 1:3%length(list_tg)
...
%     disp(list_tg(t).name)
    ...
    pcd = dir('*_sk_clrd.mat'); % loading predictand file, file ends with _sk
    load(pcd.name);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    baseFileName = sprintf('%s_pca_stp.mat','hiron_point-134a-bangladesh-uhslc.mat_sk_clrd.mat');
