...
%     end
    ...
    for ii = 1:length(wind_mat)
        ...
        % Define the grid
        lat_ind = find(lat_pred >= lat_t - 2 & lat_pred <= lat_t + 2); % find latitude within this range
        if lon_t < 0
            lon_ind = find(lon_pred >= (lon_t+360) - 2 & lon_pred <= (lon_t+360) + 2);
        else
            lon_ind = find(lon_pred >= lon_t - 2 & lon_pred <= lon_t + 2);
        end
    ...
    cd 'F:\OneDrive - Knights - University of Central Florida\Daten\MLR\TG_pct_pcd_4X4'
    ...
    cd 'F:\OneDrive - Knights - University of Central Florida\Daten\Remote Sensing\GPCP'
    gpcp_mat = dir('*.mat');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    cd 'F:\OneDrive - Knights - University of Central Florida\Daten\Remote Sensing\SST\SST_test'
    sst_mat = dir('*.mat');
    sst_daily = []; Tsst = [];
    ...
    base_path = fullfile('F:\OneDrive - Knights - University of Central Florida\Daten\MLR\TG_pct_pcd_4X4', save_as{1});
